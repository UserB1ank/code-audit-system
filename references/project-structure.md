...
└── agent-<module-name>/     # 每个子 Agent 独立工作区
    ├── skill.md             # 审计指令（主 Agent 创建）
    ├── background.md        # 独立背景文档（主 Agent 创建）
    ├── execution.log        # 执行日志（子 Agent 写入）
...
├── skill.md         # 主 Agent 创建的审计指令
├── background.md    # 主 Agent 创建的背景文档
...
```bash
# 主 Agent 创建 00-work-background.md
# 主 Agent 创建 01-module-map.md
```
...
| 主 Agent 不创建背景文档 | 主 Agent 必须先创建 `00-work-background.md` |
