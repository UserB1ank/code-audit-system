# 工作背景 - [项目名称]

> **状态: DRAFT v1** — 基于 Phase 2A 快速预扫描的初步分析结果。深度侦察（Phase 2B）完成后将更新为 FINAL 版本，包含精确攻击面映射、代码图谱追踪结果和精确模块依赖分析。

## 项目概览

**仓库地址:** [Git URL]
**分析日期:** [日期]
**分析者:** MainAgent

---

## 技术栈

### 编程语言

| 语言 | 版本 | 文件数 |
|------|------|--------|
| [如: Python] | [如: 3.9] | [数量] |
| [如: JavaScript] | [如: ES2020] | [数量] |

### 框架和库

| 框架/库 | 版本 | 用途 |
|---------|------|------|
| [如: Django] | [如: 4.2] | Web 框架 |
| [如: React] | [如: 18.2] | 前端 UI |

### 数据库

| 数据库 | 版本 | 用途 |
|--------|------|------|
| [如: PostgreSQL] | [如: 15] | 主数据存储 |
| [如: Redis] | [如: 7] | 缓存/会话 |

### 外部服务

| 服务 | 用途 |
|------|------|
| [如: AWS S3] | 文件存储 |
| [如: Stripe] | 支付处理 |

---

## 应用类型

**主要分类:** [Web 应用 / 系统服务 / GUI 应用 / 移动应用 / 其他]

**架构:** [单体 / 微服务 / 无服务器 / 混合]

**部署方式:** [Docker / 虚拟机 / 物理机 / 云原生]

---

## 关键组件

### 入口点

| 组件 | 位置 | 描述 |
|------|------|------|
| HTTP 服务器 | `server.py:main()` | 主 Web 服务器 |
| API 路由 | `api/routes.py` | REST API 端点 |
| CLI 命令 | `cmd/main.go` | 命令行接口 |

### 攻击面区域

1. **HTTP 端点** - [数量] 个接受用户输入的路由
2. **文件上传** - [位置]
3. **认证** - [登录/注册/密码重置端点]
4. **数据库** - [查询模式]
5. **外部集成** - [调用的 API]

---

## 安全相关模式

### 认证方式
[JWT / 基于会话 / OAuth / API 密钥 / 无]

### 授权模式
[RBAC / ABAC / ACL / 未检测到]

### 输入处理
[使用了清理库 / 手动清理 / 未检测到]

### 加密
[使用的库和算法]

---

## 模块划分总结

| 模块 | 文件数 | 职责 | 分配给 |
|------|--------|------|--------|
| [模块 1] | [数量] | [描述] | SubAgent-1 |
| [模块 2] | [数量] | [描述] | SubAgent-2 |
| [模块 3] | [数量] | [描述] | SubAgent-3 |

---

## 给子 Agent 的备注

### 模块专项关注

**API 模块:**
- 关注: SQL 注入、认证绕过、IDOR
- 关键文件: `api/*.py`, `handlers/*.go`

**前端模块:**
- 关注: XSS、CSRF、客户端逻辑缺陷
- 关键文件: `src/**/*.jsx`, `components/**/*.vue`

**认证模块:**
- 关注: 弱密码哈希、会话固定
- 关键文件: `auth/*.py`, `middleware/auth.*`

---

## 需详细分析的文件

### 高优先级 (直接外部输入)
1. `[file_path]` - 处理 [输入类型]
2. `[file_path]` - 管理 [敏感操作]

### 中优先级 (内部处理)
1. `[file_path]` - 处理 [数据类型]
2. `[file_path]` - 实现 [业务逻辑]

### 低优先级 (工具/支撑)
1. `[file_path]` - 提供 [工具函数]

---

## CVE 情报分析 (专项审计模式)

> ⚠️ 本章节仅在**专项审计模式**下填充。标准模式下此章节为空。
> 完整情报报告见: `workspace/02-cve-intelligence.md`

### 已知漏洞概况

- **cve-search 查询**: vendor=`[vendor]`, product=`[product]`
- **已知 CVE 总数**: [N]
- **Critical (≥9.0)**: [N] | **High (7.0-8.9)**: [N]

### Top 5 漏洞类型

| 排名 | CWE | 类型 | 数量 |
|------|-----|------|------|
| 1 | CWE-XXX | [类型名] | [N] |

### 高风险组件 (历史 CVE 高发)

| 组件 | CVE 数 | 最近 CVE | 风险等级 |
|------|--------|----------|----------|
| [组件名] | [N] | CVE-XXXX | 🔴 |

### 对各模块的情报注入摘要

| 模块 | 注入的历史 CVE | 注入的变体推测 | 优先级调整 |
|------|---------------|---------------|-----------|
| [模块] | CVE-XXXX, ... | V-XXX, ... | P0↑ / P1 / P2↓ |

---

## 下一步

1. 每个子 Agent 应阅读其模块特定的 `module-info.md`
2. 遵循分配的 `skill.md` 中的漏洞检查清单
3. 将发现报告到 `workspace/agent-<module>/report.md`
4. 发现严重漏洞时立即通知 MainAgent
5. (专项模式) 子 Agent 应额外阅读 background.md 中的 CVE 情报章节，优先检查历史变体

---

## 🔄 FINAL 版本更新说明

> 当 Phase 2B 深度侦察完成后，MainAgent 将本文档状态更新为 **FINAL**，并补充以下内容：

### 状态更新
- 文档头部状态改为 `> **状态: FINAL** — 深度侦察已完成。`
- 删除 DRAFT 版本的说明行

### 补充内容

#### ⭐ 代码知识图谱关键发现
- 图谱识别的项目架构模式
- 高扇入函数列表（被大量调用的核心逻辑）
- 高扇出函数列表（调用大量下游函数的调度器）
- 死代码区域（可能包含遗留危险逻辑）
- 完整的 HTTP 端点列表

#### 精确攻击面地图
- 每个入口函数的详细签名和认证状态
- 每个危险 Sink 的调用上下文
- 信任边界详细分析

#### 精确模块依赖
- 基于图谱的跨模块调用关系
- 模块依赖图
- 关键调用路径的图谱追踪结果
